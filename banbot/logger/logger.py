...
def get_color(color: ColorStr) -> str:
...
class Config:
 ...
 level={self._level} instance={self._INSTANCE})>"
    ...
    def update(self, **settings: int) -> None:
        ...
        """
        if all(key in self.settings for key in settings) and all(
            0 < settings[key] <= 5 for key in settings
        ):
            self._settings.update(settings)
        else:
            raise ValueError(
                f"Invalid key or value in {settings}. Remember,\
...
class MetaLogger(type):
    ...
    def __new__(self, name: str, bases: Tuple[type], attrs: Dict[str, Any]) -> type:
...
class Logger(metaclass=MetaLogger):
    ...
    def custom(
        ...
        header: str = "custom",
        *args: Any,
        color: str = get_color("reset"),
        **kwargs: Any,
    ) -> None:
        func_name = sys._getframe().f_code.co_name
        if self._runner(func_name):
            print(
                f"{color}[{header.upper()}]: {msg}{get_color('reset')}",
                *args,
                end="",
                **kwargs,
            )
    ...
    def info(self, msg: str, *args: Any, **kwargs: Any) -> None:
        ...
        if self._runner(func_name):
            print(
                f"{Color.YELLOW.value}[{func_name.upper()}]: {msg}",
                *args,
                end="",
                **kwargs,
            )
    ...
    def success(self, msg: str, *args: Any, **kwargs: Any) -> None:
        ...
        if self._runner(func_name):
            print(
                f"{Color.GREEN.value}[{func_name.upper()}]: {msg}",
                *args,
                end="",
                **kwargs,
            )
    ...
    def warning(self, msg: str, *args: Any, **kwargs: Any) -> None:
        ...
        if self._runner(func_name):
            print(
                f"{Color.RED.value}[{func_name.upper()}]: {msg}",
                *args,
                end="",
                **kwargs,
            )
    ...
    def error(self, msg: str, *args: Any, **kwargs: Any) -> None:
        ...
        if self._runner(func_name):
            print(
                f"{Color.RED_BOLD.value}[{func_name.upper()}]: {msg}",
                *args,
                end="",
                **kwargs,
            )
    ...
    def debug(self, msg: str, *args: Any, **kwargs: Any) -> None:
        ...
        if self._runner(func_name):
            print(
                f"{Color.BLUE.value}[{func_name.upper()}]: {msg}",
                *args,
                end="",
                **kwargs,
            )
