...
class Row(Shared):
    ...
    def __hash__(self) -> int:
        if not hasattr(self, "pk"):
            raise TypeError("Row cannot be hashed before saved")
        return hash(
            (
                self._table_name,
                self._db_path,
    ...
    def save(self, show_query: bool = False) -> None:
        ...
        query = f"INSERT INTO {self._table_name} {self.keys()} VALUES ({condition})"
...
class Table(Shared):
    ...
    def get_cols(self) -> Tuple[Any, ...]:
        return tuple(self._col_names[:-1])
...
class DB(Shared):
    ...
    def get_table(self, name: str, default: T = None) -> Union[Table, T]:  # type: ignore
